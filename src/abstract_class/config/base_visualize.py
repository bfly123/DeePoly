...
class BaseVisualizer:
    ...
    def generate_1d_analysis(self, 
        ...
        self._save_error_analysis_report(result_dir, analysis_data['errors'], timing_info)
        ...
        for var_idx, var_name in enumerate(variables):
            ...
            self._create_1d_training_scatter_analysis(
                analysis_data, var_idx, var_name,
    ...
    def _prepare_1d_analysis_data(self, data_train, data_test, model, fitter, coeffs, exact_solution_func):
        ...
        return {
            ...
            'predictions': {
                'net_train': net_train, 'net_test': net_test,
                'deepoly_train': deepoly_train, 'deepoly_test': deepoly_test,
                'exact_train': exact_train, 'exact_test': exact_test
            },
            'errors': errors
        }

    def _create_1d_training_scatter_analysis(self, analysis_data, var_idx, var_name, save_path):
        """Create 1D training data scatter analysis plot"""
        fig = plt.figure(figsize=(20, 12))
        gs = gridspec.GridSpec(2, 2, figure=fig, height_ratios=[3, 2], hspace=0.3, wspace=0.25)
        ...
        ax1.plot(x_sorted, exact_sol[sort_idx], 'g-', linewidth=3, label='Exact Solution', alpha=0.9, zorder=1)
        
        # Scatter points for training predictions
        ax1.scatter(x_train, net_sol, c='red', s=25, alpha=0.7, label='PINNs (Training Points)', zorder=3)
        ax1.scatter(x_train, deepoly_sol, c='blue', s=25, alpha=0.7, label='DeePoly (Training Points)', zorder=2, marker='^')
        
        ax1.set_title(f'Training Data: Solutions Comparison - {var_name}', fontsize=16, fontweight='bold')
        ...
        sc3 = ax2.scatter(x_train, y_net, c=net_error, cmap='Reds', s=40, 
                         vmin=0, vmax=vmax_err, alpha=0.8, label='PINNs Error')
        sc4 = ax2.scatter(x_train, y_deepoly, c=deepoly_error, cmap='Blues', s=40, 
                         vmin=0, vmax=vmax_err, alpha=0.8, label='DeePoly Error', marker='^')
        
        ax2.set_title(f'Training Data: Error Distribution', fontweight='bold', fontsize=16)
        ...
        ax3.semilogy(x_sorted, net_error[sort_idx], 'r-', linewidth=2.5, label='PINNs Error', alpha=0.8)
        ax3.semilogy(x_sorted, deepoly_error[sort_idx], 'b-', linewidth=2.5, label='DeePoly Error', alpha=0.8)
        ax3.set_title(f'Training Data: Error Comparison (Log Scale)', fontsize=14, fontweight='bold')
        ...
        bars1 = ax4.bar(x_pos - width/2, max_errors, width, label='Max Error', alpha=0.8, color=['red', 'blue'])
        bars2 = ax4.bar(x_pos + width/2, mean_errors, width, label='Mean Error', alpha=0.6, color=['lightcoral', 'lightblue'])
        ...
        for i, (bar1, bar2) in enumerate(zip(bars1, bars2)):
            ...
            ax4.text(bar1.get_x() + bar1.get_width()/2., height1*1.1, f'{height1:.2e}', 
                    ha='center', va='bottom', fontsize=9, rotation=45)
            ax4.text(bar2.get_x() + bar2.get_width()/2., height2*1.1, f'{height2:.2e}', 
                    ha='center', va='bottom', fontsize=9, rotation=45)
        
        plt.suptitle(f'Training Data Analysis - {var_name}', fontsize=20, fontweight='bold', y=0.96)
        plt.tight_layout()
        self._save_figure(fig, save_path)
        self._close_figure(fig)

    def _create_1d_test_line_analysis(self, analysis_data, var_idx, var_name, save_path):
        """Create 1D test data line analysis plot"""
        fig = plt.figure(figsize=(18, 12))
        gs = gridspec.GridSpec(2, 3, figure=fig, height_ratios=[2.5, 1.5], hspace=0.3, wspace=0.3)
        ...
        ax1.plot(x_sorted, exact_sorted, 'g-', linewidth=3, label='Exact Solution', alpha=0.9)
        ax1.plot(x_sorted, net_sorted, 'r--', linewidth=2, label='PINNs Solution', alpha=0.8)
        ax1.plot(x_sorted, deepoly_sorted, 'b:', linewidth=2, label='DeePoly Solution', alpha=0.8)
        ax1.set_title(f'Test Data: Solutions Comparison - {var_name}', fontsize=18, fontweight='bold')
        ax1.set_xlabel(x_label, fontsize=14)
        ax1.set_ylabel(var_name, fontsize=14)
        ax1.legend(fontsize=12)
        ax1.grid(True, alpha=0.3)
        ...
        ax2.set_title(f'PINNs Error (Max: {net_error.max():.2e})', fontsize=14, fontweight='bold')
        ...
        ax3.set_title(f'DeePoly Error (Max: {deepoly_error.max():.2e})', fontsize=14, fontweight='bold')
        ax3.set_xlabel(x_label, fontsize=12)
        ax3.set_ylabel('|Error| (log)', fontsize=12)
        ax3.grid(True, alpha=0.3)
    ...
    def generate_2d_analysis(self, 
        ...
        print(f"2D analysis completed. Generated {len(variables)} sets of training/test visualizations.")

    def _prepare_2d_analysis_data(self, data_train, data_test, model, fitter, coeffs, exact_solution_func):
    ...
    def _create_2d_training_scatter_analysis(self, analysis_data, var_idx, var_name, save_path):
        ...
        deepoly_sol = analysis_data['predictions']['deepoly_train'][:, var_idx:var_idx+1]
        exact_sol = analysis_data['predictions']['exact_train'][:, var_idx:var_idx+1]
    ...
    def _create_2d_test_grid_analysis(self, analysis_data, var_idx, var_name, save_path):
        ...
        deepoly_sol = analysis_data['predictions']['deepoly_test'][:, var_idx:var_idx+1]
        ...
        # Top row: Solution distributions (each spans 4 columns)
        ax1 = fig.add_subplot(gs[0, 0:4])
        im1 = ax1.contourf(x1_grid, x2_grid, net_grid, levels=20, cmap='RdYlBu_r', 
                          vmin=vmin_sol, vmax=vmax_sol)
        ax1.contour(x1_grid, x2_grid, net_grid, levels=10, colors='black', alpha=0.3, linewidths=0.5)
        ax1.set_title(f'PINNs Solution', fontweight='bold', fontsize=16)
        ax1.set_xlabel(x_label, fontsize=14)
        ax1.set_ylabel(y_label, fontsize=14)
        ax1.set_aspect('equal', adjustable='box')
        ...
        ax2 = fig.add_subplot(gs[0, 4:8])
        im2 = ax2.contourf(x1_grid, x2_grid, deepoly_grid, levels=20, cmap='RdYlBu_r',
                          vmin=vmin_sol, vmax=vmax_sol)
        ax2.contour(x1_grid, x2_grid, deepoly_grid, levels=10, colors='black', alpha=0.3, linewidths=0.5)
        ax2.set_title(f'DeePoly Solution', fontweight='bold', fontsize=16)
        ax2.set_xlabel(x_label, fontsize=14)
        ax2.set_ylabel(y_label, fontsize=14)
        ax2.set_aspect('equal', adjustable='box')
        ...
        ax3 = fig.add_subplot(gs[0, 8:12])
        im3 = ax3.contourf(x1_grid, x2_grid, exact_grid, levels=20, cmap='RdYlBu_r',
                          vmin=vmin_sol, vmax=vmax_sol)
        ax3.contour(x1_grid, x2_grid, exact_grid, levels=10, colors='black', alpha=0.3, linewidths=0.5)
        ax3.set_title(f'Exact Solution', fontweight='bold', fontsize=16)
        ax3.set_xlabel(x_label, fontsize=14)
        ax3.set_ylabel(y_label, fontsize=14)
        ax3.set_aspect('equal', adjustable='box')
        ...
        # Bottom row: Error distributions (centered and larger)
        ax4 = fig.add_subplot(gs[1, 1:5])
        im4 = ax4.contourf(x1_grid, x2_grid, net_error_grid, levels=25, cmap='hot',
                          vmin=0, vmax=vmax_net_err)
        ax4.contour(x1_grid, x2_grid, net_error_grid, levels=12, colors='black', alpha=0.4, linewidths=0.6)
        ax4.set_title(f'PINNs Error Distribution (Max: {vmax_net_err:.2e})', 
                      fontweight='bold', fontsize=20)
        ax4.set_xlabel(x_label, fontsize=18)
        ax4.set_ylabel(y_label, fontsize=18)
        ...
        ax5 = fig.add_subplot(gs[1, 7:11])
        im5 = ax5.contourf(x1_grid, x2_grid, deepoly_error_grid, levels=25, cmap='hot',
                          vmin=0, vmax=vmax_deepoly_err)
        ax5.contour(x1_grid, x2_grid, deepoly_error_grid, levels=12, colors='black', alpha=0.4, linewidths=0.6)
        ax5.set_title(f'DeePoly Error Distribution (Max: {vmax_deepoly_err:.2e})', 
                      fontweight='bold', fontsize=20)
        ax5.set_xlabel(x_label, fontsize=18)
        ax5.set_ylabel(y_label, fontsize=18)
    ...
    def _reconstruct_test_grid_2d(self, x_coords, values):
        ...
        for i, x1_val in enumerate(x1_unique):
            for j, x2_val in enumerate(x2_unique):
                ...
                mask = (np.abs(x_coords[:, 0] - x1_val) < 1e-10) & (np.abs(x_coords[:, 1] - x2_val) < 1e-10)
                if np.any(mask):
                    idx = np.where(mask)[0][0]
                    value_grid[j, i] = values[idx, 0]  # Note: j,i indexing for proper orientation
                else:
                    # If exact match not found, use nearest neighbor
                    distances = np.sqrt((x_coords[:, 0] - x1_val)**2 + (x_coords[:, 1] - x2_val)**2)
    ...
    def _print_error_statistics(self, net_train_errors, net_test_errors, deepoly_train_errors, deepoly_test_errors):
    ...
    def _save_error_analysis_report(self, result_dir, errors, timing_info=None):
        ...
        with open(report_path, 'w') as f:
            ...
            for method, dataset, error_data in [
                ...
                f.write(f"{method:<15} {dataset:<8} {error_data['MSE']:<12.2e} {error_data['MAE']:<12.2e} {error_data['Max Error']:<12.2e} {error_data['Relative Error']:<12.2e}\n")
            ...
            f.write("-"*40 + "\n")
            train_improvement = ((errors['net_train_errors']['MSE'] - errors['deepoly_train_errors']['MSE']) / errors['net_train_errors']['MSE']) * 100
            test_improvement = ((errors['net_test_errors']['MSE'] - errors['deepoly_test_errors']['MSE']) / errors['net_test_errors']['MSE']) * 100
            
            f.write(f"Training MSE Improvement: {train_improvement:+.2f}%\n")
            f.write(f"Test MSE Improvement: {test_improvement:+.2f}%\n")
            
            # Performance assessment
            for dataset, improvement in [("Training", train_improvement), ("Test", test_improvement)]:
    ...
    def reconstruct_grid_nd(self, data: Dict, preds: np.ndarray, config) -> tuple:
        ...
        for dim in range(n_dim):
            ...
            if len(unique_dim) != grid_dims[dim]:
                print(f"Info: Dimension {dim} - Adjusted grid size from {grid_dims[dim]} to {len(unique_dim)}")

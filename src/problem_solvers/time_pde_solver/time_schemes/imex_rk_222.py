...
class ImexRK222(BaseTimeScheme):
    ...
    def time_step(self, u_n: np.ndarray, u_seg: List[np.ndarray], dt: float) -> Tuple[np.ndarray, List[np.ndarray], np.ndarray]:
        ...
        return u_new, u_seg_new, coeffs_stage2  # 返回最后一个阶段的系数
        ...
        for segment_idx in range(self.fitter.ns):
            ...
            for stage_idx, (u_seg_stage, coeffs_stage, weight) in enumerate([
                ...
                if self.fitter.has_operator("N"):
                    N_vals = self.fitter.N_func(self.fitter._features[segment_idx], u_seg_stage)
                ...
                if self.fitter.has_operator("L2") and self.fitter.has_operator("F"):
                    ...
                    if L2_seg is not None:
                        F_vals = self.fitter.F_func(self.fitter._features[segment_idx], u_seg_stage)
            ...
            for eq_idx in range(ne):
                ...
                if L2_operators is not None and self.fitter.has_operator("F"):
                    ...
                    if (segment_idx < len(u_n_seg_list) and 
                        ...
                        # 计算F值
                        F_vals = self.fitter.F_func(self.fitter._features[segment_idx], u_seg_for_F)
    ...
    def _build_stage_rhs(self, segment_idx: int, stage: int, **kwargs) -> np.ndarray:
        ...
        if hasattr(coeffs_n, "shape") and len(coeffs_n.shape) > 2:
            ...
            coeffs_seg_n = coeffs_n.flatten()[: self.fitter.dgN]
        ...
        if stage == 1:
            ...
            for eq_idx in range(ne):
                ...
                coeffs_eq = (
                    ...
                    else coeffs_seg_n[: self.fitter.dgN]
                )
                ...
                if self.fitter.has_operator("N"):
                    ...
                    N_vals = self.fitter.N_func(self.fitter._features[segment_idx], u_n_seg)
                    if hasattr(N_vals, "__len__") and len(N_vals) == n_points:
            ...
            if hasattr(coeffs_prev, "shape") and len(coeffs_prev.shape) > 2:
                ...
                coeffs_seg_prev = coeffs_prev.flatten()[: self.fitter.dgN]
            ...
            for eq_idx in range(ne):
                ...
                coeffs_eq_prev = (
                    ...
                    else coeffs_seg_prev[: self.fitter.dgN]
                )
                ...
                if self.fitter.has_operator("N"):
                    ...
                    N_vals = self.fitter.N_func(self.fitter._features[segment_idx], u_prev)
                    if hasattr(N_vals, "__len__") and len(N_vals) == n_points:

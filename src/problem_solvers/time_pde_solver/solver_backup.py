...
class TimePDESolver:
    def __init__(self, config=None, case_dir=None):
        ...
        # Save case directory
        self.case_dir = (
            case_dir if case_dir is not None else getattr(self.config, "case_dir", None)
        )
    ...
    def _compute_adaptive_timestep(
        self, it: int, T: float, dt: float, U: np.ndarray
    ) -> float:
    ...
    def _train_neural_network_step(
        self, dt: float, U_current: np.ndarray = None
    ) -> None:

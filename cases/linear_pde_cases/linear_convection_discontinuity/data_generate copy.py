...
def generate_reference_solution(x):
    ...
    for i in range(len(x_coords)):
        ...
        # Apply boundary conditions considering the domain [0,1] x [0,1]
        # If characteristic traces back outside [0,1], apply boundary conditions
        if x_char < 0:
            # Bottom boundary condition: u = 0 at x = 0
            u[i] = 0.5
        elif x_char > 1:
            # Top boundary condition: u = 0 at x = 1  
            u[i] = 0.5
        else:
            # Use the piecewise initial condition from config.json at t=0
            u[i] = get_initial_condition(x_char)+0.5

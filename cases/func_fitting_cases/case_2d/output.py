...
def plot_solution(data: Dict, prediction: np.ndarray, save_path: Optional[str] = None):
    ...
    scatter2 = ax2.scatter(x_test[:, 0], x_test[:, 1], abs_error, 
                          c=abs_error, cmap='hot', s=30, alpha=0.6)
    ax2.set_xlabel('x')
    ax2.set_ylabel('y')
    ax2.set_zlabel('误差')
    ax2.set_title('误差散点图')
    plt.colorbar(scatter2, ax=ax2, label='|误差|')
    ax2.grid(True)
...
def plot_solution_mesh(data: Dict, prediction: np.ndarray, save_path: Optional[str] = None):
    ...
    # 2. Error surface plot
    ax2 = fig.add_subplot(122, projection='3d')
    surf2 = ax2.plot_surface(X, Y, Z_error, cmap='hot', alpha=0.8)
    ax2.set_xlabel('x')
    ax2.set_ylabel('y')
    ax2.set_zlabel('Error')
    ax2.set_title('Absolute Error')
    plt.colorbar(surf2, ax=ax2, label='|Error|')
    ax2.grid(True)
    ax2.view_init(elev=20, azim=45)
